...
function redirectToHostPagination(domain, page, fromPage) {
  ...
  if (fromPage == 'FILTER_SEARCH')
    url = domain + "venues" + "/" + region + "/" + category + "/" + capacity + "/";

  if (fromPage == '1')
    url = domain + "wedding-venues-in-chhatarpur/";

  if (fromPage == '2')
    url = domain + "wedding-venues-in-mundka/";

  if (fromPage == '3')
    url = domain + "wedding-venues-in-gt-karnal-road/";

  if (fromPage == '4')
    url = domain + "wedding-venues-in-nh8-pushpanjali/";

  if (fromPage == '5')
    url = domain + "wedding-venues-in-vaishali-vasundhara/";

  if (fromPage == '0')
    url = domain + "wedding-venues-in-delhi-ncr/";
...
function redirectToHost(domain) {
  ...
  if (region == "" && category == "") {
    $(".errorMsg").show();
    msg += "<div style='display: block;' class='errorMsg'><ul><li>Please select a valid search filter(s).</li></ul></div>";
  ...
  console.log('passed: ' + strSearch);
  location.href = domain + "venues" + strSearch;
  return false;
}
